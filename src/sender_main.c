    ...
    while (!feof(fp) && bytesToTransfer > 0) {
        ...
        while (1) {
            ...
            if (recv_bytes == -1) { 
                ...
                // printf("Timeout. Resending packets from %d\n", cummulated_ack);
                ...
                // if isACK(S+N+1), last ACK, break;
                    ...
                    // printf("Received last ack for packet %d\n", ack_num);
                    break;
                }
            }
        }
        ...
        // for (int i = 0; i < WINDOW_SIZE; i++) {
        //     // for (int j = 0; j < MAX_RESEND; j++) {
        //     //     recv_bytes = recvfrom(s, &ack_num, sizeof (ack_num), 0, (struct sockaddr *) &si_other, (socklen_t *) &slen);
        //     //     if (ack_num == seq_num) {
        //     //         printf("Received ack for packet %d\n", seq_num);
        //     //         bzero(packets[i].data, MAX_BUFFER_SIZE);
        ...
        //     while (ack_num != seq_num) {
        //         sent_bytes = sendto(s, &packet, sizeof (packet), 0, (struct sockaddr *) &si_other, slen);
        //         if (sent_bytes == -1) diep("sendto() failed");
        //         recvfrom(s, &ack_num, sizeof (ack_num), 0, (struct sockaddr *) &si_other, (socklen_t *) &slen);
        //         resend_count++;
        //         printf("packet -- %d -- resend -- %d -- times\n", seq_num, resend_count);
